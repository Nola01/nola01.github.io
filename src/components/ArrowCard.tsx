      ...
      <div class="w-full group-hover:text-black group-hover:dark:text-white blend">
        <div class="flex flex-wrap items-center gap-2">
          {pill &&
            <div class="text-sm capitalize px-2 py-0.5 rounded-full border border-black/15 dark:border-white/25">
              {entry.collection === "blog" ? "post" : entry.collection === "projects" ? "project" : "trayectoria"}
            </div>
          }
        ...
        <ul class="flex flex-wrap mt-2 gap-1">
          {"tags" in entry.data && Array.isArray(entry.data.tags) && 
            ...
            // @ts-ignore - Solución al error de Property 'key' does not exist
            <li key={tag} class="text-xs uppercase py-0.5 px-2 rounded bg-black/5 dark:bg-white/20 text-black/75 dark:text-white/75">
              {truncateText(tag, 20)}
            </li>
          ))}
        </ul>
